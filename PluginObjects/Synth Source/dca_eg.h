...
\brief Provides parameters to the DCA class, recieved mostly from the GUI, and modulators from other objects.
...
struct DCAParameters
{
	...
	bool mute = false;			///< A mute switch, implemented in dca_eg.cpp to set gainRaw = 0 if mute = 1
	double panValue = 0.0;		///< Ranges from [-1, +1], meaning [left -> right], initially center
};
...
public:
	...
	bool processSynthAudio(SynthProcessorData* audioData)
	{
		...
		// --- mono -> stereo: copy left channel to right and apply panning gain
...
enum class egState
{
	kOff, kDelay, kAttack, kDecay, kSustain, kRelease, kShutdown, kHoldOn, kHoldOff
};
enum class egType
{
	kADSR, kAHDSR, kAHR
...
public:
	...
	{
		return state;
	} ///< returns current state of the EG finite state machine

// --- output EG identifer access
	bool isOutputEG()
	{
		return outputEG;
	} ///< returns true if this EG is connected to the output DCA

	void setIsOutputEG(bool b)
	{
		outputEG = b;
	} ///< set/clear the output DCA flag
