...
const ModOutputData SynthLFO::renderModulatorOutput()
{
	...
	double lfo1DelayTimeSamples = msecToSamples(sampleRate, parameters->delay_mSec);

	lfo1Timer.setTargetValueInSamples(lfo1DelayTimeSamples);
	if (!lfo1Timer.timerExpired())
	{
		lfo1Timer.advanceTimer();
	}
	...
	// --- scale by amplitude, where fade happens
