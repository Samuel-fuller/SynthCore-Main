...
#include "Synth Source/synthcore.h" //VS filter used.
...
enum controlID {
	...
	lfo1Frequency_Hz = 0,
	uEnablePortamento = 1,
	uPortamentoTime_mSec = 11,
	uWaveformSelect = 2,
	uPulseWidth_pct = 3,
	lfo1Mode = 20,
	lfo1DelayTime_mSec = 30,
	lfo1RampTime_mSec = 40,
	lfo1OutputAmplitude = 50
};
...
private:
	...
	double uPortamentoTime_mSec = 0.0;
	double uPulseWidth_pct = 0.0;
	double lfo1DelayTime_mSec = 0.0;
	double lfo1RampTime_mSec = 0.0;
	double lfo1OutputAmplitude = 0.0;

	// --- Discrete Plugin Variables 
	int uEnablePortamento = 0;
	enum class uEnablePortamentoEnum { SWITCH_OFF,SWITCH_ON };	// to compare: if(compareEnumToInt(uEnablePortamentoEnum::SWITCH_OFF, uEnablePortamento)) etc... 

	int uWaveformSelect = 0;
	enum class uWaveformSelectEnum { AnalogSaw_WT,Parabola,Sin,WT_1 };	// to compare: if(compareEnumToInt(uWaveformSelectEnum::AnalogSaw_WT, uWaveformSelect)) etc... 
